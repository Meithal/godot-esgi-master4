...
public partial class Root : Node2D
{
	...
	// signature now includes `passes` as final double argument
	private delegate int PredictDelegate(double flappyHeight, double flappyX, double verticalSpeed, double distToRoof, double obs_dx, double obs_y, double passes);
	...
	private void StartFlappy()
	{
		...
		// create unique demo filename per play
		try {
		...
		if (_useNativeModel) {
			...
			if (_nativePredictFunc != null) {
				...
				try {
					...
					pred = _nativePredictFunc(_output.FlappyHeight, _output.FlappyX, verticalSpeed, distToRoof, nearest_dx, nearest_y, (double)passesArg);
					humanJump = pred == 1;
					// throttle logging to avoid spamming output
					try {
						long now = DateTime.UtcNow.Ticks / TimeSpan.TicksPerMillisecond;
						if (now - _lastNativeLogMs > 500) {
							GD.Print($"[native] pred={pred} fh={_output.FlappyHeight:F3} fx={_output.FlappyX:F3} vs={verticalSpeed:F3} distToRoof={distToRoof:F3} dx={nearest_dx:F3} oy={nearest_y:F3}");
